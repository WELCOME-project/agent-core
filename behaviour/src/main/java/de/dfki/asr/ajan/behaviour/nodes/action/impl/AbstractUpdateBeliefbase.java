...
public abstract class AbstractUpdateBeliefbase extends AbstractChainStep {
	...
	private void updateTarget(final Action action, final Model addModel, final Model removeModel) {
		...
		} else if (action.getTargetBase() != null && action.getTargetBase().toString().equals(AJANVocabulary.LOCAL_SERVICES_KNOWLEDGE.toString())) {
			action.getObject().getLocalServicesBeliefs().update(addModel, removeModel, false);
		} else if (action.getTargetBase() != null && action.getTargetBase().toString().equals(AJANVocabulary.LOCAL_AGENTS_KNOWLEDGE.toString())) {
			action.getObject().getLocalAgentsBeliefs().update(addModel, removeModel, false);
		} else {
