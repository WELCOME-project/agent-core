...
public final class StateLoader {
	...
	private static Repository getInitializedRepository(final URI url, AgentTaskInformation taskInfos)
		...
		} else if (url.equals(new URI(AJANVocabulary.LOCAL_SERVICES_KNOWLEDGE.toString()))) {
			repo = taskInfos.getLocalServicesBeliefs().getInitializedRepository();
		} else if (url.equals(new URI(AJANVocabulary.LOCAL_AGENTS_KNOWLEDGE.toString()))) {
			repo = taskInfos.getLocalAgentsBeliefs().getInitializedRepository();
		} else {
